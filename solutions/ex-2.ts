...
test("working with discriminated unions", () => {
  ...
  function describe(
    ...
    return drink.type === "coffee" ? drink.roast : drink.style;
  }
...
test("combining unions & intersections", () => {
  ...
  function announcePetDetail(pet: Pet) {
    const descriptor = pet.animalType === "cat" ? pet.coloration : pet.size;
    return `This is the ${pet.familyName} family ${pet.animalType}, ${pet.name} the ${descriptor} ${pet.breedName}.`
  }
